...
# Fix break when MDE/Nonf changed after test results entered 
```
...
observeEvent(input$loadShortcut, {
  ...
  if (!is.na(shortcutList[12])) {
    r1 <- strsplit(shortcutList[12],";", fixed = TRUE)
    r2 <- r1[[1]]
    r3 <- list()
    r4 <- list()
    for (i in 1:length(r2)) {
      r3 <- strsplit(r2[i],"|",fixed = TRUE)
      r3 <- as.numeric(c(r3[[1]]))
...
Row {data-height=50}
-----------------------------
v2.3 - Thanks for coming, for version history, to report bugs and submit feature requests [click here](https://github.com/alphanumerritt/sequential-test-app/issues){target="_blank"}.
