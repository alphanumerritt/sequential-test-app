...
observeEvent(c(input$alpha, input$pwr, input$cvra, input$nonf, input$mde, input$tls, input$checknum, reactiveResultsVars$tgl),{ #}, reactiveResultsVars$rslt),{
  ...
  if (length(reactiveResultsVars$rslt) > 0) {
    ...
    for (i in 1:length(reactiveResultsVars$rslt)) {
      ...
      reactiveResultsVars$status <- if (boundary == "_middle" || boundary == "_upper") paste0(complete_text,boundary) else paste0(complete_text,boundary,tls_text)
      reactiveResultsVars$win$checkpoint <- chk
      reactiveResultsVars$win$outcome <- switch(reactiveResultsVars$status,
      ...
      if (reactiveResultsVars$win$outcome == "reject") {
        break
      } 
    }
  }
...
observeEvent(reactiveResultsVars$win$outcome, {
  req(!is.null(reactiveResultsVars$win$outcome))
  ...
  if (reactiveResultsVars$win$outcome == "reject") {
    ...
    textConf <- if (reactiveResultsVars$win$outcome == "reject") paste0(conf,'%') else NULL
