...
getz <- function (a,b,c,d,e) {
  round(-testBinomial(x1=a, x2=b, n1=c, n2=d, delta0 = a/c*e), digits = 2)
}
```
...
* Fix quick load, make elements optional
* Fix if input results N is greater than next checkpoint, it breaks
* Format everything!!!
...
div(
  div(class = "ql", "To load a prior configuration: "),
  div(class = "ql", textInput("shortcut", NULL, width = "130px")),
  h4(class = "ql", actionLink("loadShortcut", label = "Load Shortcut")),
)
```
...
```{r}
output$currentCvr = renderText({paste(input$conversions/input$traff*100,"%")})

```


### Inputs help
`Confidence level` - This is ($1 - \alpha$) where $\alpha$ is your nominal false positive error rate.
...
#### How many times do you want to peek on the test results? 
(Include final analysis)
```{r}
numericInput(
  "checknum",
  label = NULL,
...
observeEvent(c(input$alpha, input$pwr, input$cvra, input$nonf, input$mde, input$tls, input$checknum, reactiveResultsVars$rslt),{
  ...
  if (length(reactiveResultsVars$rslt) > 0) {
    for (i in 1:length(reactiveResultsVars$rslt)) {
      ...
      currZ <- currRslt[[1]][6]
      reactiveDesignVars$table$complete[currChk, 5] = currZ
    }
    ...
    for (i in 1:length(reactiveResultsVars$rslt)) {
      ...
      a <- as.numeric(d[[1]][1])
      b <- d[[1]][6]
      if (b > testDesign$upper$bound[a]) {
        reactiveResultsVars$win <-
          list(checkpoint = a, outcome = "efficacy")
        break
      } else if (b < testDesign$lower$bound[a]) {
        reactiveResultsVars$win <-
...
div(class='three-c',
  h5('Conversion rate'),
  div(textOutput("aRate", inline = TRUE)
      )
...
div(class='three-c',
  h5('Conversion rate'),
  div(textOutput("bRate", inline = TRUE)
      )
...
```{r}
div(class = "cent",
    div(class = "slider", htmlOutput("checkSelect")),
    h4(class = "slider", actionLink("addResults", "Add checkpoint results")))
```
...
output$bRate <- renderText({paste0(input$bConversions / input$bTraffic * 100, "%")})

output$aRate <- renderText({paste0(input$aConversions / input$aTraffic * 100, "%")})
...
```{r}
  ...
  div(class = "ztable", tableOutput("resultTable"))
...
output$resultTable <- renderTable({
  ...
  reactiveDesignVars$table$complete
...
output$outcome <- renderText({
  ...
  if (reactiveResultsVars$win$outcome == "efficacy") {
    return("The test experince has passed the efficacy boundary.")
  } else {
...
output$boundPlot <- renderPlot({
  ...
  days <- round(input$traff/input$dayNum)
  df$estDays <- round(df$samples / days)
