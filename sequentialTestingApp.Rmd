...
# Enable selection of single analysis?
...
### Help
`Confidence level` - This is ($1 - \alpha$) where $\alpha$ is your nominal false positive error rate when the true effect size is at the null hypothesis threshold.

`Power` - This is ($1 - \beta$) where $\beta$ is your nominal false negative error rate. It represents your likelihood of getting a statisticaly significant positive result when the true effect size is the Minimum Detectable Effect.

##### More coming soon!

```{r advancedInputs, include=FALSE}
# TO BE DEVELOPED
# ### Advanced
...
#### Checkpoint selection: Which analysis do these results apply to?
```{r}
div(class = "slider",
