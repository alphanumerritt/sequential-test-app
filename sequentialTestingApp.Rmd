...
# k_checks <- 8
# n_sequential <- 10000
# h <- list("checkpoint1" = c(2,200,1000,210,1100), "checkpoint5" = c(4,600,5000,680,5300)) 
# 
# 
# dr <- data.frame(chkpt = c(1:k_checks)) %>% 
#   mutate(planned_n = chkpt*(1/k_checks*n_sequential), 
...
```{r scratchpad2tim, include=FALSE}
# #library(tidyverse)
# 
# # Step 1. Let's call this "the plan"
# dr <- data.frame(chkpt = c(1:8), planned_n = seq(1000,8000,1000),actual_n = 0)
# 
# # Step 2. Data comes in after in any sequence and some rows will be 
# # missing, but will be guaranteed that each row will be > any previous 
# # rows (except those where no data has been entered)
# input <- "dr[c(8),3] <- c(6000)"
...
observeEvent(input$addResults, {
  ...
  current_n_ttl <- input$aTraffic + input$bTraffic
  n_valid <- TRUE
  row_check <- max(inchk - 1,1)
  
  # Make sure n > prior checkpoint n
  if (length(global_results$rslt) > 0 && inchk != 1) {
    for (i in 1:row_check) {
      row_check_name <- paste0("checkpoint",i)
      prior_n_ttl <- if (!is.null(global_results$rslt[[row_check_name]])) global_results$rslt[[row_check_name]][3] + global_results$rslt[[row_check_name]][5] else 1
      if (prior_n_ttl >= current_n_ttl) {
        n_valid <- FALSE
      }
    }
  }
  
  # MAKE SURE N < ANY FUTURE CHECKPOINT N ALREADY ENTERED ----
  
  # Only update results vars if valid data
  if (n_valid) {
    global_results$rslt[[rownum]] <- rsltList
    global_results$tgl <- global_results$tgl * -1
  } else {
    showNotification("Oops! Looks like you may have already entered this data for an earlier checkpoint, or maybe got the wrong traffic numbers. Please check and re-enter the data.", type="error", duration = 10)
  }
...
```{r results_table_output, include=FALSE}

output$resultTable <- render_gt({
  req(input$numVars == 0)
  gt(global_design$table$complete) %>%
...
```{r clear_results_observer, include=FALSE}
# Clear all the reactive results variables (list)
observeEvent(input$clear, {
  req(!is.null(global_results$rslt))
...
observeEvent(c(global_results$win$outcome,global_design$table), {
  req(!is.null(global_results$win$outcome))
  
  # Set key variables needed
  checkNum <- paste0("checkpoint",global_results$win$checkpoint)
  checkNumInt <- global_results$win$checkpoint
  ciZ <- global_design$table$upper[checkNumInt] # upper boundary z-score from original design for the most recent check-in point
  xA <- global_results$rslt[[checkNum]][2] # number of conversions in control
  xB <- global_results$rslt[[checkNum]][4] # number of conversions in test
  nA <- global_results$rslt[[checkNum]][3] # number of samples in control
  nB <- global_results$rslt[[checkNum]][5] # number of samples in test
  cvra <- xA/nA
  cvrb <- xB/nB
  
  # Calculate confidence if null rejected
  if (global_results$win$outcome == "reject") {
    # Set variables
    nList <- global_design$table$samples/2
    winZ <- global_results$rslt[[checkNum]][6]
    ...
    Zs <- if (checkNumInt == 1) winZ else c(global_design$table$upper[1:(checkNumInt-1)],winZ) # upper boundary z-scores from test design up to most recent check-in, original z-scores for all but the most recent which is calculated z-score 
    tls <- global_design$tls # tls number of tails
    ...
    # Make the text link for confidence level
    textConf <- if (global_results$win$outcome == "reject") paste0(conf,'%') else NULL
    global_results$win$conf <- textConf 
  ...
  # Set the reactive variable for conf interval
  global_results$win$ciData <- ci
...
output$results_default <- renderText({
  req(is.null(global_results$win$ciData))
...
output$ci_plot <- renderPlot({
  ...
  ci <- global_results$win$ciData
  
  ci_labels <- c(paste0(round(ci$lower*100,2),"%"),paste0(round(ci$mean*100,2),"%"),paste0(round(ci$upper*100,2),"%"))
  stat_sig_text <- if (!is.null(global_results$win$conf[[1]]))  paste0("Statistically significant at ", global_results$win$conf[[1]], " confidence") else " "
  chart_title <- paste0(input$alpha,"% confidence interval of effect (B/A-1)")
...
output$advice <- renderText({
  req(!is.null(global_results$status))
  
  message <- switch(global_results$status,
...
As results are recorded, the calculated test statistic (z-score) will appear on this chart. Boundaries will reset to fit sample sizes at each checkpoint.

```{r boundaryplot_output, include=FALSE}
output$boundPlot <- renderPlot({
  req(input$numVars == 0)

  df <- global_design$table$complete
  ...
  ggplot(df, aes(x = samples)) +
    ...
    geom_vline(xintercept = global_design$fixedn, size=.25, linetype = "dashed") +
    annotate(geom = "text", y = 1,
         x = global_design$fixedn * 1.02, 
         label= "Fixed horizon sample size",
         fontface = "bold",
         angle = 90,
         size = 4.0) +
