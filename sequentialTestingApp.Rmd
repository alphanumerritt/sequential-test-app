...
1. **Before** committing to a sequential test, check the expected duration with a fixed design. It's healthy to run tests for 2 weeks to capture a few business cycles.
2. The previous version of this calculator allowed you to easily change the timing of the checkpoints vs. spacing them evenly. That step is now unnecessary since **boundaries and checkpoints will automatically adjust** whenever you enter results. 
    + To be explicit, you are not bound to the *timing* of the check-ins, only to the number of checkins.
3. **Use the quick-load shortcuts!** It will save you a ton of time, especially after you have entered results.
4. **Use the Power Plot** to help you plan your test configuration. The probabilities of false positives and false negatives at different effect sizes can prove very informative. Try adding a **non-inferiority margin** and see what happens!
