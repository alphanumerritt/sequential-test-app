...
observeEvent(reactiveResultsVars$win$outcome, {
  ...
  textLci <- paste0(round(ci$lower*100, digits=2),'%')
  textUci <- paste0(round(ci$upper*100, digits=2),'%')
  ...
  reactiveResultsVars$win$ciLow <- textLci
  reactiveResultsVars$win$ciHigh <- textUci
...
output$ci_plot <- renderPlot({
  ...
  stat_sig_text <- paste0("Statistically significant at ",reactiveResultsVars$win$conf[[1]]," confidence")
  
  ggplot(ci, aes(x=mean, y=1)) +
    annotate("text",
             x=ci$mean,
             y=1.8,
             label="Confidence interval of effect",
             fontface = "bold",
             size = 5) +
    annotate("text",
